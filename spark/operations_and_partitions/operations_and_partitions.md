...
Figure 2: A Spark transformation that creates a new column named <i>group</i> (figure by author)**.
</p>
...
For the code in Snippet 2, Spark creates the pipeline backward as follows.
...
The benefit of doing it backward can be explained as follows. While executing the code, the Driver node identifies that in step 3, there is a `groupBy` operation, which converts the pipeline as follows:
...
Steps 1-4 belong to the first stage and the rest of the steps belong to the second stage. When the code is executed once, the driver node knows that the output of the shuffle writes is already available to the executors, for a reasonable period of time. If the code is executed more times within that duration, it will execute from the second stage directly, which improves runtime. Furthermore, if caching is used at the stage at any transformation operation, it will execute the tasks only after that operation.

## Further Reading

For more in-depth understanding, check out the following materials:
